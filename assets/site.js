(() => {
  ...
  const equations = [
    ...
    { lines: ["L_CE = −∑_i y_i", "log p_θ(i|x)"] },
    { lines: ["r_t = π_θ(a_t|s_t)", "/ π_old(a_t|s_t)"] },
    { lines: ["L_PPO = E[min(r_t A_t,", "clip(r_t,1−ε,1+ε)A_t)]"] },
    { lines: ["A_i = (R_i − μ_R)", "/ (σ_R + ε)"] },
    { lines: ["J_GRPO = E[∑_i ρ_i A_i/G", "− β D_KL(π||π_ref)]"] },
    { lines: ["q(x_t|x₀) = N(", "√ā_t x₀, (1−ā_t)I)"] },
    { lines: ["L_diff = E[||ε −", "ε_θ(x_t,t)||²]"] },
    { lines: ["∇_θ J = E[", "∇ log π_θ(a|s) · A]"] },
    { lines: ["A_t^GAE = ∑_l", "(γλ)^l δ_{t+l}"] },
    { lines: ["δ_t = r_t + γ V(s_{t+1})", "− V(s_t)"] },
    { lines: ["D_KL(p||q) = ∑_x p(x)", "log[p(x)/q(x)]"] },
    { lines: ["H(π) = −∑_a π(a|s)", "log π(a|s)"] },
    { lines: ["p(x_t|x_{<t}) =", "softmax(W h_t)"] },
    { lines: ["h_l = LN(h_{l-1} +", "Attn(h_{l-1}))"] },
    { lines: ["FFN(x) = W₂ σ(", "W₁x + b₁) + b₂"] },
    { lines: ["W' = W + (α/r) BA"] },
    { lines: ["L_NCE = −log", "e^(sim(z,z+)/τ) / ∑_j e^(...)"] },
    { lines: ["V*(s) = max_a E[", "r + γ V*(s')]"] },
    { lines: ["s_θ(x,t) ≈", "∇_x log p_t(x)"] },
    { lines: ["dx_t = [f − ½g²s_θ]dt", "+ g(t) dW_t"] },
    { lines: ["z = E_φ(x)", "x' = D_ψ(z)"] },
    { lines: ["I(X;Z) = E log", "p(x,z) / (p(x)·p(z))"] },
    { lines: ["Q*(s,a) = E[r +", "γ max_{a'} Q*(s',a')]"] },
    { lines: ["L_SFT = −E_{x,y}", "∑_t log π_θ(y_t|x,y_{<t})"] },
